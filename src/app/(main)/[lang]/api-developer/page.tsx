...
export default function ApiDeveloperPage(props: { params: Promise<{ lang: string }> }) {
  ...
  const handlePrepareActivation = (e: React.FormEvent) => {
    e.preventDefault();
  ...
  const handleActivateApi = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        setToastMessage({ type: "success", text: data.message || "تم تفعيل API فوراً بنجاح" });
        ...
        setSiteName(data.user?.apiSiteName || siteName.trim());
        setSiteUrl(data.user?.apiSiteUrl || siteUrl.trim());
        setShowActivationConfirmation(false);
      } else {
        setToastMessage({ type: "error", text: data.error || "حدث خطأ أثناء تفعيل API" });
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
      ...
      {userSession?.role === "admin" && (
        <div className="mb-6 rounded-2xl p-4 border border-amber-500/30 bg-amber-500/10 flex items-start gap-4">
          <span className="material-symbols-outlined text-amber-400 text-2xl shrink-0 mt-0.5">shield</span>
          <div className="text-sm space-y-1">
            <p className="font-bold text-amber-300">
              {lang === "ar" ? "وضع المدير العام (Master Admin Mode)" : "Master Admin Mode"}
            </p>
            <p className="text-on-surface-variant text-xs leading-relaxed">
              {lang === "ar" 
                ? "أنت مسجل حالياً بحساب الإدارة. يمكنك الاطلاع على بيانات التوثيق والأكواد والروابط أدناه، أو تفعيل الـ API لحسابك للاختبار والتجربة، أو استخدام حساب عميل عادي للربط الفعلي."
                : "You are logged in as Master Admin. You can view all documentation and endpoints below, or enable API for testing, or use a client account for production."}
            </p>
          </div>
        </div>
      )}
            ...
            {isApiEnabled ? (
              ...
              <div className="flex flex-col items-center justify-center p-6 text-center rounded-2xl bg-amber-500/10 border border-amber-500/30">
                <div className="w-12 h-12 rounded-full bg-amber-500/20 flex items-center justify-center mb-3">
                  <span className="material-symbols-outlined text-2xl text-amber-400">bolt</span>
                </div>
                <p className="text-sm font-bold text-amber-400 mb-1">
                  {lang === "ar" ? "جاهز للتفعيل الفوري" : "Ready for Instant Activation"}
                </p>
                <p className="text-xs text-amber-400/80">
                  {lang === "ar" ? "أكد بيانات موقعك وسيتم التفعيل الآن دون انتظار الإدارة." : "Confirm your site details to activate now without admin approval."}
                </p>
              </div>
            )}
          </div>
          ...
          {!isApiEnabled && (
            <div className="glass-card rounded-3xl p-6 border border-purple-500/30 relative overflow-hidden">
              ...
              <h2 className="text-lg font-bold text-on-surface flex items-center gap-2 mb-4 relative z-10">
                ...
                {lang === "ar" ? "تفعيل API الآن" : "Activate API Now"}
              </h2>
                ...
                <div className="space-y-4 relative z-10">
                  <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 space-y-3">
                    <div className="flex items-center gap-2 text-amber-400 font-bold text-sm">
                      ...
                      <span>{lang === "ar" ? "تأكيد التفعيل الفوري" : "Confirm Instant Activation"}</span>
                    </div>
                    <div className="text-xs text-on-surface-variant space-y-2">
                      <p><span className="font-bold text-on-surface">{lang === "ar" ? "الموقع: " : "Site: "}</span>{siteName.trim()}</p>
                      <p className="dir-ltr text-left break-all"><span className="font-bold text-on-surface">URL: </span>{siteUrl.trim()}</p>
                    </div>
                    <p className="text-[11px] text-amber-400/90">
                      {lang === "ar" ? "بالتأكيد سيتم إنشاء مفتاح API وتفعيله لحسابك فوراً." : "Confirming will create and activate an API key for your account immediately."}
                    </p>
                  </div>
                  <button
                    type="button"
                    ...
                    className="w-full bg-purple-500 text-white py-3 rounded-xl font-bold text-sm hover:bg-purple-600 transition-all flex items-center justify-center gap-2 shadow-md disabled:opacity-50"
                  >
                    {isSubmitting ? (
                    ...
                    <span>{lang === "ar" ? "تأكيد وتفعيل API الآن" : "Confirm and Activate Now"}</span>
                  </button>
                  <button
                    type="button"
                    disabled={isSubmitting}
                    onClick={() => setShowActivationConfirmation(false)}
                    className="w-full py-2.5 rounded-xl border border-outline-variant/30 text-on-surface-variant hover:text-on-surface hover:bg-surface-container-high text-xs font-bold transition-all disabled:opacity-50"
                  >
                    {lang === "ar" ? "تعديل البيانات" : "Edit Details"}
                  </button>
                </div>
              ) : (
                <form onSubmit={handlePrepareActivation} className="space-y-4 relative z-10">
                  ...
                  <button
                    type="submit"
                    className="w-full bg-purple-500 text-white py-3 rounded-xl font-bold text-sm hover:bg-purple-600 transition-all flex items-center justify-center gap-2 shadow-md mt-2"
                  >
                    <span className="material-symbols-outlined text-base">arrow_forward</span>
                    <span>{lang === "ar" ? "متابعة للتأكيد" : "Continue to Confirmation"}</span>
                  </button>
                </form>
              )}
            </div>
