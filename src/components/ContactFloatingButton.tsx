...
const SOCIAL_LINKS: SocialLink[] = [
  ...
  },
  {
    ...
    color: "#7c3aed",
    bg: "linear-gradient(135deg, #7c3aed 0%, #059669 100%)",
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
        <path d="M12.012 2c-5.506 0-9.989 4.478-9.99 9.984 0 1.762.459 3.48 1.332 5.001L2 22l5.148-1.348c1.472.802 3.13 1.224 4.863 1.225h.005c5.505 0 9.989-4.478 9.99-9.985 0-2.668-1.039-5.176-2.927-7.062A9.923 9.923 0 0 0 12.012 2zm.005 16.521h-.004c-1.493 0-2.957-.401-4.232-1.157l-.304-.18-3.146.825.839-3.067-.198-.315c-.832-1.323-1.272-2.859-1.272-4.436 0-4.492 3.656-8.147 8.152-8.147 2.176 0 4.221.848 5.76 2.387a8.096 8.096 0 0 1 2.384 5.763c0 4.493-3.656 8.147-8.152 8.147zm4.469-6.108c-.245-.123-1.452-.716-1.677-.798-.225-.082-.389-.123-.553.123-.164.246-.635.798-.778.962-.143.164-.286.184-.531.062-.245-.123-1.037-.382-1.976-1.219-.73-.651-1.223-1.455-1.366-1.7-.143-.246-.015-.379.108-.501.111-.11.245-.286.368-.429.123-.143.164-.246.245-.409.082-.164.041-.307-.02-.429-.062-.123-.553-1.332-.757-1.822-.204-.49-.409-.419-.553-.429h-.471c-.164 0-.429.062-.654.307-.225.246-.86.84-.86 2.05 0 1.209.88 2.373 1.002 2.537.123.164 1.733 2.646 4.198 3.712.586.254 1.044.406 1.401.52.59.187 1.127.16 1.551.097.473-.07 1.452-.593 1.656-1.166.204-.573.204-1.064.143-1.166-.061-.102-.225-.164-.47-.287z"/>
      </svg>
  ...
  },
  {
    ...
    color: "#a78bfa",
    bg: "linear-gradient(135deg, #059669 0%, #7c3aed 100%)",
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
    ...
    >
      ...
      <div
        ...
        className={`relative flex flex-col sm:flex-row items-center gap-1.5 sm:gap-2.5 group cursor-pointer select-none touch-none ${
          isDragging ? "scale-110 drop-shadow-[0_20px_35px_rgba(124,58,237,0.8)]" : "hover:scale-105 active:scale-95"
        }`}
      >
        {/* Interactive Speech Bubble Prompt (Mobile & Desktop Responsive) */}
        {!open && (
          <div className="flex flex-col items-center sm:items-start bg-[#0b0f19]/95 backdrop-blur-xl border border-violet-500/50 rounded-2xl px-2.5 py-1 sm:px-3.5 sm:py-2 shadow-[0_8px_25px_rgba(0,0,0,0.6)] group-hover:border-violet-400 transition-all pointer-events-none mb-1 sm:mb-0">
            ...
            <div className="hidden sm:flex text-[9px] sm:text-[10px] text-violet-300 font-medium items-center gap-1 mt-0.5">
              <span>{isAr ? "تواصل مباشر" : "Live Chat"}</span>
              <span className="bg-violet-500/20 text-violet-300 px-1 rounded font-mono text-[8px]">7 قنوات</span>
            </div>
          </div>
        )}

        {/* Full-Body Vector Support Mascot Character */}
        <div className="relative w-14 h-18 sm:w-20 sm:h-24 flex items-center justify-center filter drop-shadow-[0_6px_16px_rgba(124,58,237,0.5)] group-hover:drop-shadow-[0_10px_24px_rgba(124,58,237,0.8)] transition-all">
          
          {/* Glowing Base Platform */}
          <div className="absolute bottom-0.5 w-10 sm:w-14 h-3 rounded-full bg-violet-500/30 blur-md animate-pulse pointer-events-none"></div>

          {open ? (
            /* Close Trigger Circular Badge when Opened */
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-gradient-to-tr from-rose-600 to-red-500 border-2 border-rose-400/70 shadow-2xl flex items-center justify-center text-white font-bold text-xl sm:text-2xl animate-spin-once">
              ✕
            </div>
          ) : (
            ...
            >
              ...
              {/* ── 2. HEAD & FACE ── */}
              {/* Cute Cap / Hair */}
              <path d="M32 38 C32 20 88 20 88 38 Z" fill="#065f46" stroke="#7c3aed" strokeWidth="1.5" />
              
              {/* Face */}
              <rect x="30" y="30" width="60" height="42" rx="18" fill="#0f172a" stroke="#7c3aed" strokeWidth="2.5" />
              
              {/* Visor Area */}
              <rect x="36" y="38" width="48" height="22" rx="9" fill="#064e3b" stroke="#a78bfa" strokeWidth="1" />

              {/* Left Eye (Blinking) */}
              <ellipse cx="48" cy="48" rx="4" ry="5.5" fill="#a78bfa">
                <animate
                  attributeName="ry"
              ...
              {/* Right Eye (Blinking) */}
              <ellipse cx="72" cy="48" rx="4" ry="5.5" fill="#a78bfa">
                <animate
                  attributeName="ry"
                  values="5.5;5.5;0.5;5.5;5.5"
                  dur="3.2s"
              ...
              <path d="M25 48 Q28 66 42 64" stroke="#7c3aed" strokeWidth="2.5" strokeLinecap="round" fill="none" />
              <circle cx="44" cy="64" r="3" fill="#22c55e" className="animate-pulse" />

              {/* ── 3. NECK & COLLAR ── */}
              <rect x="53" y="71" width="14" height="6" rx="2" fill="#1e293b" stroke="#7c3aed" strokeWidth="1" />
              ...
              />
              {/* Shirt V-Neck & Tie */}
              <path d="M52 77 L60 88 L68 77 Z" fill="#0f172a" stroke="#a78bfa" strokeWidth="1" />
              <polygon points="58,88 62,88 61,102 59,102" fill="#7c3aed" />

              {/* Support ID Badge */}
              <rect x="42" y="86" width="10" height="12" rx="2" fill="#0f172a" stroke="#6ee7b7" strokeWidth="1" />
              <line x1="44" y1="90" x2="50" y2="90" stroke="#a78bfa" strokeWidth="1" />
              <line x1="44" y1="93" x2="48" y2="93" stroke="#6ee7b7" strokeWidth="1" />

              {/* ── 5. LEFT ARM (WAVING / GESTURING) ── */}
              <g className="origin-[34px_80px]">
                <circle cx="34" cy="80" r="4.5" fill="#047857" stroke="#7c3aed" strokeWidth="1.5" />
                ...
                {/* Waving Hand */}
                <circle cx="18" cy="46" r="5" fill="#a78bfa" stroke="#ffffff" strokeWidth="1.5">
                  <animateTransform
                    attributeName="transform"
                    type="rotate"
                    values="0 34 80; 12 34 80; -8 34 80; 0 34 80"
              ...
              <circle cx="86" cy="80" r="4.5" fill="#047857" stroke="#7c3aed" strokeWidth="1.5" />
              ...
              {/* Cute Tech Sneakers */}
              <rect x="42" y="132" width="14" height="7" rx="3.5" fill="#7c3aed" stroke="#ffffff" strokeWidth="1" />
              <rect x="66" y="132" width="14" height="7" rx="3.5" fill="#7c3aed" stroke="#ffffff" strokeWidth="1" />

              {/* Energy Thruster Glow Under Shoes */}
              <ellipse cx="49" cy="142" rx="8" ry="2.5" fill="#a78bfa" className="animate-pulse" />
              <ellipse cx="73" cy="142" rx="8" ry="2.5" fill="#a78bfa" className="animate-pulse" />
            </svg>
          )}
        </div>
      </div>
      ...
      {open && (
        ...
        >
          ...
          <div className="px-3 py-1 rounded-xl bg-surface-container-high/95 border border-violet-500/30 backdrop-blur-lg text-[10px] sm:text-[11px] font-bold text-violet-400 flex items-center gap-1.5 shadow-lg whitespace-nowrap">
            ...
            return (
              ...
              >
                ...
                <a
                  ...
                  className="w-11 h-11 sm:w-12 sm:h-12 rounded-2xl flex items-center justify-center text-white transition-all duration-300 transform active:scale-75 shadow-lg group relative border border-violet-400/25 hover:rotate-6"
                  style={{
                    background: item.bg,
